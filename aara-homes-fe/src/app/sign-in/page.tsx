...
export default function SignIn() {
  ...
  const login = (payload: any) => {
    return axios.post(`${getBaseUrl}/voter/auth`, {
      email: payload.email,
      ...
      <Flex justifyContent={'space-between'} width={"100%"}>
        ...
        <Box width={"45%"}>     
          ...
          <Box mr={"12em"} mt={"4em"}>
            ...
            <Box>
              <Text fontWeight={"500"} fontSize={"28px"} color={"#FFFFFF"} fontFamily={"Georgia Pro Cond"}>Sign In</Text>
              ...
              <Text fontSize={"14px"} color={"#FFFFF0"}  fontWeight={"500"} mt={"1em"}>SIGN IN WITH EMAIL ADDRESS</Text>
            </Box>
            ...
            <Flex justifyContent={"space-between"}>
              <Box mt={"2em"}>
                <Text color={"#F5F5F580"} fontSize={"12px"}>New to AaraHomes?<br/> 
                <chakra.span color={"#fff"} cursor={"pointer"} onClick={() => location.replace("/sign-up")}>Sign up for an account</chakra.span></Text>
              </Box>

              <Button my={"2em"} bgColor={"brand.primary"} color={"#3A3A3A"} width={"30%"} borderRadius={"none"} variant={"primary"} fontSize={"16px"} type="submit" isLoading={isLoading} isDisabled={!isFormValid}>Sign In</Button>
              </Flex>
            </form>
          </Box>
      </Flex>
